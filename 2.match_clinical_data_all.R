...
# format ID_short in clinical data
clinical_temp['sessionID'] <- sub("C1", "C4", clinical_temp$sessionID)
intersectIDs = intersect(clinical_temp$sessionID,unique(CBT_followup$subjID))
...
write.csv(CBT_followup_matched, 
          ...
          row.names = FALSE)
clinical_CBT_followup = subset(clinical_temp, clinical_temp$sessionID %in% intersectIDs)
