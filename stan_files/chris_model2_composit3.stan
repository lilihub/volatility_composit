...
transformed parameters {
  ...
  for(i in 1:N){
    ...
    beta[i] = inv_logit(mu_pr[4] + sigma[4]*beta_pr[i])*10;
    gamma[i] = inv_logit(mu_pr[5] + sigma[5]*gamma_pr[i]);
  }
}
model {
  // Hyperparameters
  ...
  for (i in 1:N) {
    ...
    for (t in 1:Tsubj[i]) {
      ...
      A = inv_logit(A_base[i] + A_vs[i]*status[i,t]+ A_gb[i]*out[i,t]);
...
generated quantities{
  ...
  mu_A_vs = inv_logit(mu_pr[3]);
  mu_beta = inv_logit(mu_pr[4]) * 10;
  mu_gamma = inv_logit(mu_pr[5]);
