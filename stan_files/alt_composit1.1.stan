...
// The difference of this model compared with 'alt_composit1.stan' is the 
// encoding method of outcome_blue variable. blue 
...
data {
  ...
  real outcome_blue1[N, T];  // blue steals -> 0, blue not steal -> 1
...
model {
  ...
  for (i in 1:N) {
    ...
    for (t in 1:Tsubj[i]) {
      ...
      // Bernoulli distribution requires the input to be one dimension and 
      // choice should be coded as 1 (e.g. the blue one was chosen)
      // and 0 (the blue one was not chosen))
      // no - sign before beta[i] -> outcome_blue1
      // with - sign before beta[i] -> outcome_blue
      choiceB[i, t] ~ bernoulli_logit(beta[i] * gvalue);
      // prediction error of the blue choice
      PE = outcome_blue1[i, t] - ev;
