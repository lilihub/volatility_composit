...
// covariates for learning rate components and the other two parameters
...
data {
  ...
  int<lower=-1, upper=1> status[N, T];
  int<lower=-1, upper=1> out[N,T];
...
parameters {
  ...
  row_vector[nC] A_base_cov;    // baseline learning rate
  row_vector[nC] A_vs_cov; // volatile stable learning rate
  row_vector[nC] A_gb_cov; // good bad learning rate
  row_vector[nC] A_vs_gb_cov;
  row_vector[nC] gamma_cov;
  row_vector[nC] beta_cov;
}
...
transformed parameters {
  ...
  for(i in 1:N){
    A_base[i] = mu_pr[1] +  sigma[1]*A_base_pr[i] + sum(A_base_cov .*cov[i,]);
    A_vs[i] = mu_pr[2] + sigma[2]*A_vs_pr[i] + sum(A_vs_cov .*cov[i,]);
    A_gb[i] = mu_pr[3] + sigma[3]*A_gb_pr[i] + sum(A_gb_cov .*cov[i,]);
    A_vs_gb[i] = mu_pr[4] + sigma[4]*A_vs_gb_pr[i] + sum(A_vs_gb_cov .*cov[i,]);

    beta[i] = inv_logit(mu_pr[5] + sigma[5]*beta_pr[i] + sum(beta_cov .*cov[i,]))*10;
    gamma[i] = inv_logit(mu_pr[6] + sigma[6]*gamma_pr[i] + sum(gamma_cov .*cov[i,]));
  }
}
model {
  // Hyperparameters
  ...
  beta_pr ~ normal(0, 1);
  gamma_pr ~ normal(0, 1);
  A_base_cov ~ normal(0, 1);
  A_vs_cov ~ normal(0, 1);
  A_gb_cov ~ normal(0, 1);
  A_vs_gb_cov ~ normal(0, 1);
  gamma_cov ~ normal(0, 1);
  beta_cov ~ normal(0, 1);
