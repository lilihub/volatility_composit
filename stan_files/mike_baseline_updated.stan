...
model {
  ...
  for (i in 1:N) {
    ...
    ev = initV_ev;
    for (t in 1:Tsubj[i]) {
      fvalue[1] = fmax(fmin((gamma[i]*(ev - 0.5) + 0.5), 1), 0); // fvalue of the blue choice (magnitudes of point loss)
      fvalue[2] = fmax(fmin((gamma[i]*(1 - ev - 0.5) + 0.5), 1), 0); // fvalue of the orange choice  (magnitudes of point loss)
      //fvalue[2] = 1 - fvalue[1];
