...
model {
  ...
  for (i in 1:N) {
    ...
    ev = initV_ev;
    for (t in 1:Tsubj[i]) {
      fvalue[1] = fmax(fmin((gamma[i]*(ev - 0.5) + 0.5), 1), 0); // fvalue of the blue choice
      fvalue[2] = fmax(fmin((gamma[i]*(1 - ev - 0.5) + 0.5), 1), 0); // fvalue of the orange choice
      //fvalue[2] = 1 - fvalue[1];
      gvalue[1] = fvalue[1] * amount_blue[i, t]; // gvalue of the blue choice
      gvalue[2] = fvalue[2] * amount_orange[i, t]; // gvalue of the orange choice
