...
model {
  ...
  for (i in 1:N) {
    ...
    for (t in 1:Tsubj[i]) {
      ...
      gvalue = gamma[i] * (ev[i] - (1 - ev[i])) + (1 - gamma[i]) * (amount_blue[i,t] - amount_orange[i, t]);
  ...
  { // local section, this saves time and space
    for (i in 1:N) {
      ...
      for (t in 1:Tsubj[i]) {
        ...
        gvalue = gamma[i] * (ev[i] - (1 - ev[i])) + (1 - gamma[i]) * (amount_blue[i,t] - amount_orange[i, t]); // gvalue of the blue choice
