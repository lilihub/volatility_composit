...
## also for plotting one schedule posterior of multi_block model
plot_posterior <- function(groups, all_fits, color_scheme,name, model_type){
  ...
  h_all <- plot_grid(title, plot_row, ncol = 1, # rel_heights values control vertical title margins
                     rel_heights = c(0.1, 1))
  ggsave(filename=paste0("plots/posterior/", name, "_pos", ".pdf"), plot = h_all, width = 5, height = 5)
  return(h_all)
}
...
plot_individual_rain <- function(schedule1_stable_fit, schedule1_volatile_fit, schedule2_stable_fit, schedule2_volatile_fit, name){
  ...
  title <- ggdraw() + draw_label(name, fontface = 'bold', x = 0, hjust = 0) +
    theme(plot.margin = margin(0, 0, 0, 7))
  h_all = plot_grid(title, all_plot, ncol = 1, rel_heights = c(0.5, 5)) # rel_heights values control title margins
  ggsave(filename=paste0("plots/individual/", name, "_rain", ".pdf"), plot = h_all, width = 6, height = 5)
  return(h_all)
}

## This function is used to plot the individual-level changes
## (A, beta, and gamma) from stable to volatile block, **two schedules** 
## and two conditions (baseline and followup) in the same plot

plot_combine_change <- function(baseline_stable_fit, baseline_volatile_fit, followup_stable_fit, followup_volatile_fit, name){
  A = c(colMeans(baseline_stable_fit$A), colMeans(followup_stable_fit$A),
        colMeans(baseline_volatile_fit$A), colMeans(followup_volatile_fit$A))
  beta = c(colMeans(baseline_stable_fit$beta), colMeans(followup_stable_fit$beta),
           colMeans(baseline_volatile_fit$beta), colMeans(followup_volatile_fit$beta))
  gamma = c(colMeans(baseline_stable_fit$gamma), colMeans(followup_stable_fit$gamma),
            colMeans(baseline_volatile_fit$gamma), colMeans(followup_volatile_fit$gamma))
  df = data.frame(A = A,
                  beta = beta,
                  gamma = gamma,
                  condition = rep(c('stable', 'volatile'), 
                                  each = c(ncol(baseline_stable_fit$A)+ncol(followup_stable_fit$A))),
                  combine = rep(c('Bstable', 'Fstable','Bvolatile','Fvolatile'), 
                                time = c(ncol(baseline_stable_fit$A),
                                         ncol(followup_stable_fit$A),
                                         ncol(baseline_volatile_fit$A),
                                         ncol(followup_volatile_fit$A))),
                  paired = rep(1:(ncol(baseline_stable_fit$A)+ncol(followup_stable_fit$A)), time = 2))
  df$combine = factor(df$combine, levels = c('Bstable', 'Bvolatile', 'Fstable', 'Fvolatile'))
  p1 <- ggplot(df,aes(x=combine,y=A,fill=condition,col=condition))+
    geom_point(aes(color=condition),size=2) +
    geom_line(aes(group = paired),color="grey") +
    scale_fill_brewer(palette = "Dark2")+
...
plot_combine_rain <- function(baseline_stable_fit, baseline_volatile_fit, followup_stable_fit, followup_volatile_fit, name){
  A = c(colMeans(baseline_stable_fit$A), colMeans(followup_stable_fit$A), 
        colMeans(baseline_volatile_fit$A), colMeans(followup_volatile_fit$A))
  beta = c(colMeans(baseline_stable_fit$beta), colMeans(followup_stable_fit$beta), 
           colMeans(baseline_volatile_fit$beta), colMeans(followup_volatile_fit$beta))
  ...
  all = data.frame(A = A,
                   ...
                   gamma = gamma,
                   condition = rep(c('stable', 'volatile'), 
                                   each = ncol(baseline_stable_fit$A)+ncol(followup_stable_fit$A)),
                   combine = rep(c('Bstable','Fstable','Bvolatile','Fvolatile'), 
                                 time = c(ncol(baseline_stable_fit$A),
                                          ncol(followup_stable_fit$A),
                                          ncol(baseline_volatile_fit$A),
                                          ncol(followup_volatile_fit$A))),
                   paired = rep(1:(ncol(baseline_stable_fit$A)+ncol(followup_stable_fit$A)), time = 2))
  all$combine = factor(all$combine, levels = c('Bstable', 'Bvolatile', 'Fvolatile', 'Fstable'))
  p1 <- ggplot(all,aes(x=combine,y=A,fill=condition,col=condition))+
    geom_flat_violin(position = position_nudge(x = .2, y = 0), alpha = .4,adjust =4)+
    geom_point(position=position_jitter(width = .15),size = 0.5, alpha = 0.4)+ylab('A')+
    scale_fill_brewer(palette = "Dark2")+
...
plot_individual_change_multi_block1 <- function(schedule1_fit, name){
  
  A = c(colMeans(schedule1_fit$A[,,1]), 
        colMeans(schedule1_fit$A[,,2]))
  beta = c(colMeans(schedule1_fit$beta[,,1]), 
           colMeans(schedule1_fit$beta[,,2]))
  gamma = c(colMeans(schedule1_fit$gamma[,,1]), 
            colMeans(schedule1_fit$gamma[,,2]))
  df = data.frame(A = A,
                  beta = beta,
                  gamma = gamma,
                  condition = rep(c('stable', 'volatile'), 
                                  each = ncol(schedule1_fit$A[,,1])),
                  combine = rep(c('1stable', '1volatile'), 
                                time = c(ncol(schedule1_fit$A[,,1]),
                                         ncol(schedule1_fit$A[,,2]))),
                  paired = rep(1:(ncol(schedule1_fit$A[,,1])), time = 2))
  df$combine = factor(df$combine, levels = c('1stable', '1volatile'))
  p1 <- ggplot(df,aes(x=combine,y=A,fill=condition,col=condition))+
    geom_point(aes(color=condition),size=2) +
    stat_summary(fun.data = "mean_se", geom = "errorbar", width=0.2, size=1, color = 'black') +
    geom_line(aes(group = paired),color="grey", linetype = "dashed", size = 0.5) +
